...
class SceneManager:
    """Владеет стеком сцен. Наружу отдаёт только текущую (верхнюю) сцену —
    многослойная отрисовка (например, паузы поверх геймплея) решается
    внутри самой сцены-оверлея, а не здесь. См. PauseScene."""
